...
class Qwen35Runtime:
    ...
    def generate(self, messages: list[dict[str, Any]], max_tokens: int, temperature: float) -> dict[str, Any]:
        ...
        if not rendered.endswith(no_think_suffix):
            raise RuntimeError("Unexpected Qwen3.5 no-thinking prompt suffix")
        rendered = rendered[:-len(no_think_suffix)]
        ...
        # Never return a generated role transition or echoed prompt.
        stop_markers = (
            "<|im_end|>",
            "<|im_start|>",
            "\nuser\n场景：",
        ...
        cut_positions = [
            ...
            if raw_text.find(marker) >= 0
        ]
        if cut_positions:
            raw_text = raw_text[:min(cut_positions)]
